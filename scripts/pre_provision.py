...
async def main():
    ...
    credential = AzureDeveloperCliCredential(tenant_id=os.getenv("AZURE_AD_TENANT_ID"))
    ...
    print("Updating azd env with AZURE_AUTH_APP_ID, AZURE_AD_CLIENT_ID, AZURE_AD_CLIENT_SECRET")
    update_azd_env("AZURE_AUTH_APP_ID", app.id)
    update_azd_env("AZURE_AD_CLIENT_ID", app.app_id)
    update_azd_env("AZURE_AD_CLIENT_SECRET", client_secret)
